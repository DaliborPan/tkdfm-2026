...
import { FormTabsProvider } from "./form-tabs-provider";
import { FormTabsTrigger } from "./form-tabs-trigger";
import { type FormTabsTriggerItem } from "./types";
...
> & {
  ...
  triggers: (FormTabsTriggerItem | undefined | null | false)[];
};
...
export function FormTabs({
  ...
  const providerValue = useMemo(
    () => ({
      tabGroupId: id,
      triggers: triggers.filter(Boolean) as FormTabsTriggerItem[],
      value,
  ...
  return (
    <FormTabsProvider {...providerValue}>
      <Root
        value={value}
        onValueChange={setValue}
